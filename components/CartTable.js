...
import {contextStore} from "../utils/Store"
 ...
 function BasicTable({cartData}) {
  ...
  return (
    ...
    <h2 style={{padding:'0 0 0 20px',marginBottom:"4px"}}>Cart Items.</h2>
        ...
        <TableBody>
          {cartData.map((row) => (
            ...
            >
              ...
              <TableCell>
              {/* select */}
                <Select value={row.quantity} onChange={(e)=>updateCartHandler(row,e.target.value)} >
                  {[...Array(row.countInStock).keys()].map(x=><MenuItem key={x+1} value={x+1} >{x+1}</MenuItem>)}
                </Select>
                
              </TableCell>
              <TableCell>{row.price}$</TableCell>
              <TableCell>
                  <Button  variant="contained" color="secondary" onClick={()=>deleteCartHandle(row)} >x</Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
